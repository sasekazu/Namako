...
### Model Preparation
- Before running the simulation, you need a tetrahedral mesh for deformation analysis. You have two options:
  - Generate a tetrahedral mesh from a **triangular mesh** such as OBJ or STL. In this guide, we assume you start from a triangular mesh.
  - Use a **tetrahedral mesh** generated by Gmsh in **.msh (Version 2 ASCII)** format. See [Using pre-generated tetrahedral mesh](#using-pre-generated-tetrahedral-mesh).
- For your first test, you can use `lowpoly-bunny.obj` in the `SampleResources` folder, as a triangular mesh.
- The triangular mesh is expected to:
  - be **watertight**, as required by the collision detection algorithm implemented in Namako.
  - have **sufficiently fine resolution** to reproduce visually plausible deformations. For example, an 8-vertex cube does not provide adequate resolution. If you want to remesh the model to obtain a finer resolution, you can use tools such as the Remesh modifier in Blender.
...
- Mesh generation
  - Put a triangular mesh model (such as obj) into the scene hirarchy
